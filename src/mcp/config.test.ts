...
describe("loadMcpConfig", () => {
  ...
  it("loads from $SWARM_CODER_CONFIG_DIR/mcp.json when present (highest priority)", async () => {
    ...
    writeJson(path.join(cwd, ".swarm-coder"), "mcp.json", {
  ...
  it("falls back to <cwd>/.swarm-coder/mcp.json when env not set", async () => {
    const expected = writeJson(path.join(cwd, ".swarm-coder"), "mcp.json", {
  ...
  it("preserves priority order: env > cwd/.swarm-coder > cwd/.claude > home/.claude", async () => {
  ...
  it("throws on malformed JSON", async () => {
    const p = path.join(cwd, ".swarm-coder", "mcp.json");
