/**
 * compact-prompts — byte-exact Claude Code v2.1.198 compaction prompt strings.
 *
 * Extracted from the shipped Claude Code binary (see docs/48-compaction-design.md
 * and docs/39-codex-parity-gap-analysis.md §11). Do NOT reword: Claude-family
 * models are RLVR-trained against these exact strings, and the alignment goal
 * is that openswarm trajectories are indistinguishable from Claude Code ones.
 *
 * Composition (mirrors Claude Code's T$n / reactive builders):
 *   full    = GUARD + FROM_PROMPT [+ "\n\nAdditional Instructions:\n" + custom] + REMINDER
 *   reactive =        RECENT_PROMPT [+ "\n\nAdditional Instructions:\n" + custom] + REMINDER
 */

/** No-tools guard prepended to the full-compaction summary request. */
export const COMPACT_TOOL_GUARD =
 ...
 */
export const COMPACT_FROM_PROMPT =
...
/**
 * Reactive-variant prompt ("RECENT portion of the conversation") — used when
 * earlier retained context is being kept verbatim and only newer messages are
 * summarized (keep-recent / group compaction).
 */
