...
describe("ScriptedTestEngine — in-memory script", () => {
  ...
  it("throws when no script source provided", () => {
    const saved = process.env.SWARM_CODER_TEST_SCRIPT;
    delete process.env.SWARM_CODER_TEST_SCRIPT;
    expect(() => new ScriptedTestEngine()).toThrow("ScriptedTestEngine");
    if (saved !== undefined) process.env.SWARM_CODER_TEST_SCRIPT = saved;
  });
});
...
describe("ScriptedTestEngine — scriptPath", () => {
  ...
  it("reads from SWARM_CODER_TEST_SCRIPT env when no opts provided", async () => {
    const script: ScriptedEvent[] = [
    ...
    const saved = process.env.SWARM_CODER_TEST_SCRIPT;
    process.env.SWARM_CODER_TEST_SCRIPT = tmpFile;
    ...
    try {
      ...
      if (saved !== undefined) {
        process.env.SWARM_CODER_TEST_SCRIPT = saved;
      } else {
        delete process.env.SWARM_CODER_TEST_SCRIPT;
      }
    }
