...
describe("loadHooksConfig", () => {
  ...
  it("loads from $OPENSWARM_CONFIG_DIR/hooks.json (highest priority)", async () => {
  ...
  it("falls back to <cwd>/.openswarm/hooks.json when env not set", async () => {
    const expected = writeJson(path.join(cwd, ".openswarm"), "hooks.json", {
  ...
  it("throws on malformed JSON", async () => {
    const p = path.join(cwd, ".openswarm", "hooks.json");
