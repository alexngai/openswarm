 ...
 *
 * Precedence: user aliases in `~/.openswarm/settings.json` (`aliases: {...}`)
...
/**
 * Built-in aliases shipped with openswarm. Values are pinned to model ids
 ...
 * Users can always override via `~/.openswarm/settings.json`.
 */
...
export async function loadAliases(settingsPath?: string): Promise<AliasTable> {
  const resolved = settingsPath ?? path.join(os.homedir(), ".openswarm", "settings.json");
