...
import { canonicalRoot, isWithin } from "../kernel/workspace-authority.js";
...
export async function scanProvenance(root: string): Promise<Provenance> {
  ...
  //
  // Shared with `TrustStore`, which used to spell this as a bare `path.resolve`.
  // A scan and a grant that disagree about a root's name is a grant that cannot
  // be found again.
