...
import type { ProviderError } from "../../src/core/types.js";
import type { LanguageModel } from "ai";
import { type RetryPolicy, DEFAULT_RETRY_POLICY } from "../../src/engine/retry-policy.js";
import {
  isRetryableError,
...
function isRetryableTransportError(err: unknown): boolean {
  // fallbackCode: "transport" preserves the former engine/retry-policy.ts
  // semantics this module was written against (unrecognized error → retryable
  // transport failure) after the Phase 2.3 classifier consolidation.
  const classified = classifyProviderError(err, { fallbackCode: "transport" });
