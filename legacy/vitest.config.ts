...
// and swarm.test.ts:363). Real-git tests are also load-sensitive: subprocess
// spawns from inside a fork worker cost 10-20x more under an uncapped storm
// (observed in merge-queue.test.ts, whose drain tests then crest testTimeout).
// Leaving CPU headroom keeps the subprocess tests honest without meaningfully
// slowing the fast unit suite. Override with TEST_MAX_FORKS.
//
// Vitest 4 removed `poolOptions` (the old `poolOptions.forks.maxForks` was
// silently ignored, which un-capped the suite and brought the fork storm
// back); the cap now lives in the top-level `maxWorkers` option.
...
export default defineConfig({
  test: {
    ...
    // experimental/ holds archived-but-revivable modules (see
    // experimental/BACKLOG.md); their tests keep running so the code
    // stays importable when a feature is brought back.
    include: ["src/**/*.test.ts", "test/**/*.test.ts", "experimental/**/*.test.ts"],
