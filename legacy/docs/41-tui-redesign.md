...
> **Status (GitHub #23 — attach path + persistence + real task_* landed):** the three deferrals above are now implemented:
> - **Attach to a detached daemon** — `openswarm team attach <name>` launches the interactive REPL and tails the daemon's `events.jsonl` (the blessed follow contract; there is deliberately no socket subscribe RPC — see `events-log.ts`) via `src/swarm/daemon-swarm-view.ts`. `DaemonSwarmViewProjector` feeds each recorded lane event through the shared `SwarmViewTranslator`; `tailDaemonSwarmView(eventsPath, sink)` wires it onto the live file (fs.watch + poll fallback, mirroring `team watch`). The attach source is combined with the local live host via `combineSwarmSources` so in-session `agent` spawns still render. The daemon orchestrator is never a `worker_spawned` member, so its direct children render as tree roots without needing an `orchestratorId`.
> - **Cross-turn persistence** — the swarm merge (`mergeTurnWithSwarm`) is now applied ONCE around the whole multi-turn REPL stream (was per-turn), so the subscription and its stateful `SwarmViewTranslator` live for the entire session: long-lived members persist across turns and events keep flowing while the REPL is idle between prompts.
> - **Real `task_*` emission** — `StandaloneHost.spawn()` now emits real `task_created` (with the prompt) + `task_updated` (owner assignment) lane events from the default spawn path, and `task_completed`/`task_failed` on worker exit. The `SwarmViewTranslator` translates these directly into `task_update` view events (registry status → view status). The worker-lifecycle synthesis is retained; the TaskBoard reducer keys on `taskId` so real + synthesized updates converge idempotently. Because `task_*` are recorded (not live-only), they reach the daemon's `events.jsonl` and thus the attach path.
>
> **Caveat:** the interactive REPL requires a TTY + Bun (OpenTUI); `team attach` inherits that requirement.
