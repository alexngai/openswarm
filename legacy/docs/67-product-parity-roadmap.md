...
**Unblocking the install found a fourth defect on its first green run**, in the fixture that guards the audit/history partition. That fixture patches a copy of `contracts.ts` in a temporary directory and expects the compiler to reject the unclassified event, and it invoked the compiler as `npx tsc`. The probe deliberately lives outside the checkout so it cannot disturb modules other tests are importing — which also means `npx` walks up from it and finds no local `typescript`, so it fetched the unrelated abandoned `tsc` package from the registry and ran that instead. That exits non-zero with installation advice, so the fixture's first assertion — a non-zero status — was satisfied by a program that had never read the file under test. Only the assertion on the error's *content* caught it. It passed locally because this machine happens to have a global `typescript` on `PATH`, and passed in the Linux container for the same reason; the CI runner has neither. It now invokes `node_modules/typescript/bin/tsc` by absolute path and asserts the compiler exists before trusting its exit code, which also takes the fixture off the network.

The shape of that one is worth keeping: a test that reaches for a tool by name gets whichever tool the ambient environment supplies, and the strength of the assertion is what decides whether a substitution is noticed. Had it only checked the exit status, it would have gone green forever while proving nothing.
