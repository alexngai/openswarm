...
1. `swarm-coder prompt "..."` on a TTY enters interactive ink REPL; turn completes; prompt returns to idle; user can type next turn. State transitions visible in a status bar.
...
15. Hooks: with `.swarm-coder/hooks.json` declaring a `PreToolUse` command, running a tool invokes the command, honors exit codes (0 allow, 2 deny, other fail), merges `updatedInput` / `systemMessage` / `permissionDecision` from stdout JSON.
...
24. Config path hierarchy resolves `.swarm-coder/hooks.json` before `~/.claude/settings.json`; `config_resolved` lane event captures the winning path at startup.
...
All config files (`.swarm-coder/hooks.json`, `.swarm-coder/mcp.json`, plugin directories, etc.) are resolved using a consistent hierarchy. First-match-wins:

1. `$SWARM_CODER_CONFIG_DIR/<file>` (env override)
2. `<cwd>/.swarm-coder/<file>`
...
9.2. `src/hooks/config.ts` — load hook config from `.swarm-coder/hooks.json` (with fallback to `~/.claude/settings.json`'s `hooks` field so existing Claude Code hook configs light up free). Validate via Zod.
...
- The SDK's `hooks` field accepts **JavaScript callback functions**, not shell-command config objects. Shell-command configs live in `.swarm-coder/hooks.json` (preserved for user authoring / Claude Code format compatibility); our runtime wraps them in JS callbacks before handing off to the SDK.
- Exit-code translation (0/2/other) happens inside our JS wrapper, not in the SDK.

9.5. `.swarm-coder/hooks.json` example:
```json
{
  "PreToolUse": [
...
- **Phase 2:** `npx vitest run src/ui/repl/` green (pure reducer tests); `npx swarm-coder prompt "hi"` in a real terminal shows the new REPL
- **Phase 3:** every slash command's test passes; manually entering `/help` in the REPL shows the full list
- **Phase 4:** Tier 1 tool tests green; `npx swarm-coder prompt "fetch https://example.com"` actually fetches live
- **Phase 5:** `/compact` emits an observable compaction event in the JSONL stream
- **Phase 6:** `npx swarm-coder doctor` + plugin fixture present → plugin tool appears in `/help`-style listing; invoking the model with "use shell-plugin's tool" actually calls it
