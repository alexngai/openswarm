...
- `swarm-coder swarm run tasks.jsonl --concurrency N` CLI subcommand
...
- **Worker-state file** at `.swarm-coder/workers/<agentId>.json` → M3 alongside file-backed TaskRegistry. `docs/05-swarm-model.md` §2.3 describes it, but it's not needed for M1's in-memory model.
...
1. `swarm-coder swarm run tasks.jsonl --concurrency 3` parses a JSONL task file, spawns workers, exits 0 when all complete.
...
11. **(11a)** Worker environment inheritance — unit-testable: `SWARM_CODER_AGENT_ID`, `SWARM_CODER_PARENT_PID`, `SWARM_CODER_ORCHESTRATOR_PID`, `SWARM_CODER_DEPTH`, and (when spawned from the `agent` tool) `SWARM_CODER_PARENT_TOOL_USE_ID` are set in the child process env. Verified by a test-mode worker that echoes its env back via IPC.
...
3.3. `src/swarm/ipc/parent-transport.ts` — runs inside the **worker**. Wraps `process.stdin` / `process.stdout`:
  ...
  - **Backpressure + bounded outbound queue.** `send()` and `notify()` return `Promise<void>`; resolve only after `process.stdout.write()` returns `true` OR the `drain` event fires. Queue cap: 1 MiB. When full:
    - Low-priority notifications (`lane_event`, `heartbeat`) dropped with a stderr warning: `"[swarm-coder] worker outbound queue full; dropping <method>"`.
...
4.1. `src/swarm/subprocess-spawner.ts` — `spawnWorker(request: SpawnRequest): ChildProcess`:
  - Uses `child_process.spawn` with:
    ...
    - `env`: inherits `process.env` + overrides:
      - `SWARM_CODER_AGENT_ID` — `crypto.randomUUID()`; stable for worker's lifetime; never reused.
      - `SWARM_CODER_PARENT_PID`: `process.pid`
      - `SWARM_CODER_ORCHESTRATOR_PID`: top-level orchestrator pid
      - `SWARM_CODER_DEPTH`: authoritative child depth (computed by orchestrator per §0.4)
      - `SWARM_CODER_PARENT_TOOL_USE_ID`: optional; set only when spawned from the `agent` tool (per §0.5)
      - `SWARM_CODER_TEST_SCRIPT`: optional path for tests
      - *(note: `SWARM_CODER_SESSION_ID` is NOT set — resume is out of M1 scope; the SDK assigns a fresh session id per `query()` call)*
...
4.4. `src/cli/worker-entry.ts` — the `--worker` mode entrypoint:
  ...
  - Runs `ClaudeAgentSdkEngine` (or `ScriptedTestEngine` if `SWARM_CODER_TEST_SCRIPT` is set).
  - Stamps every outbound event with `parentToolUseId` (if `SWARM_CODER_PARENT_TOOL_USE_ID` is set) per §0.5.
...
5.1. `src/swarm/orchestrator.ts` — `Orchestrator` class:
  ...
  - **Results write-failure handling.** On `results.jsonl` write error callback: emit `error` lane event via `eventsOut`, log to stderr `"[swarm-coder] failed to persist result for task <id>: <err>"`, increment `resultWriteFailures`. On first write failure: stop accepting new results, drain remaining workers to completion, exit non-zero with summary `"N task results failed to persist"`.
...
Behavior:
  ...
  - Orchestrator's depth check: if `childDepth > MAX_DEPTH` (default 3, override via `SWARM_CODER_MAX_DEPTH`), rejects with `{ status: "error", message: "recursion depth limit reached (<MAX_DEPTH>)" }`. Error message interpolates the runtime limit so tests using override values don't flap.
...
6.3. `src/swarm/depth-limit.ts` — `DEFAULT_MAX_DEPTH = 3` constant, override via `SWARM_CODER_MAX_DEPTH` env. The enforcement happens inside `SwarmHost.spawn()` impls (per §0.4) using the orchestrator's authoritative depth map — NOT inside the tool. The `agent` tool is advisory; the host is the gate.
...
7.1. `src/engine/test-engine.ts` — `ScriptedTestEngine implements AgentEngine`:
  - Loads a JSON file from `SWARM_CODER_TEST_SCRIPT`.
...
7.2. Integration test harness: `test/integration/swarm.test.ts`:
  ...
  - Sets `SWARM_CODER_TEST_SCRIPT` to a fixture file under `test/fixtures/worker-scripts/`.
...
- **Phase 4:** manual subprocess spawn: `SWARM_CODER_AGENT_ID=test-1 node dist/cli.js --worker --agent-id=test-1 </dev/null` emits `worker_ready` then exits after stdin close.
...
- **rev 2 (2026-04-20):** critic review returned REVISE with 5 critical + 7 major findings. Applied E1–E16:
  ...
  - Dropped `SWARM_CODER_SESSION_ID` from M1 env list (M7)
