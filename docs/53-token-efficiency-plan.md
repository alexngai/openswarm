...
| TE-4 | REVISED: project instructions were already budgeted (4K/file, 12K total, cross-level dedup — parity) and load once into the system prompt at startup (static per session → cacheable). The audit's "unbounded" claim was wrong | `src/engine/project-instructions.ts` | Already bounded | — | no change needed |
