...
|----|-------------|-------|-----------------|------|--------|
| TE-1 | Hard aggregate byte budget on memory block (default ~16KB, configurable), priority-ordered providers (playbooks > skills > archive > minimem), drop-with-log on overflow | `src/memory/lifecycle.ts` (`enrichTurnInputs`, `formatMemoryFragments`), coordinator | Bounds worst case (today: 0–100KB+/turn/worker) | Dropped fragments could lose useful recall — log what's dropped | todo |
| TE-2 | Fragment model: marker-delimited memory blocks injected as user-message fragments; replace-on-change with stale-copy pruning (Codex `WorldStateSection` pattern); system prompt stays byte-stable | `src/memory/lifecycle.ts`, engine turn assembly | Restores prompt-cache hits on static prefix (~10x price difference on prefix tokens); stops per-turn re-injection of unchanged fragments | Larger change; must keep `surfacedSkills` exposure attribution intact for sessionlog | todo |
| TE-3 | Rotate/cap minimem daily log | `src/memory/providers/minimem-provider.ts` | Stops unbounded growth | Low | todo |
| TE-4 | Cache + budget AGENTS.md/project instructions per session (Codex: 32KB `project_doc_max_bytes`) | `src/engine/project-instructions.ts` | Bounds 0–12KB/worker re-read | Low | todo |
