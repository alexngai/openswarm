...
| **The self-modification gate has never been measured** — rung 5 admits a change when `npm run presubmit` exits 0, and everything downstream inherits that judgement, but we have exactly two observations of it: one correct pass, and one FALSE NEGATIVE on a correct edit (the `OPENSWARM_LIVE` recursion). We have never once observed it reject a bad change, because we have never given it one. A gate with poor recall does not announce itself — it admits silently and surfaces later as merged breakage. Designed as a mutation-testing experiment in [docs/04](04-gate-discrimination.md), with pre-registered thresholds and a containment class (approval bypass, SHA-check defeat, conflicted-merge-reported-as-merged) that must be killed 100% or rung 5 does not run unattended. Notably this is the experiment JIT-Agent (arXiv:2608.25593) never ran on its own `Valid_Π`; ours is measurable because our gate is a test suite | needs no model calls, no benchmark, and no reward signal — it is the cheapest question on the critical path and the one everything else assumes | before rung 5 runs unsupervised |
