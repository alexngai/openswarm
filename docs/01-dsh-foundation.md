...
| **A failing confidence gate reports a bare 0 with no diagnosis** — `defaultConfidenceRunner` discards stdout and stderr, so a rejected tier yields only `confidence 0`, and the feedback threaded into the NEXT tier is the generic "the verification commands did not pass" with no detail on WHICH command failed or why. Diagnosing the false negative above meant reproducing the gate by hand. This also materially weakens the escalation loop the cascade exists for: tier 2 is told it failed but not how, so it is guessing | capturing the failing command's output into the tier feedback is the single highest-value improvement to command-gated cascades, and is a contained change to the runner plus the feedback string | next cascade touch |
