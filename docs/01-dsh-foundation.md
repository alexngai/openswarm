...
| ~~Cannot hot-reload our plugin dist~~ **done (2026-08-28, rung 6)** — and the previous entry here was WRONG in a way worth recording. It blamed `dsh-web-app` disabling the `hmr` row upstream, concluding the reload loop was blocked pending upstream. The real cause was ours and local: `cordis-plugin-hmr` resolves `root` against `base`, and `base` defaults to `ctx.baseUrl` — the PROFILE directory, not the working directory. So `openswarm-dev`'s `root: ['.']` watched `~/.openswarm/profiles/openswarm-dev/`, a tree of config files plus a `node_modules` of symlinks the default `ignored: ['**/node_modules']` excludes. The watcher never saw this repo, so an edit produced no change event, no reload, no `loader.exit()`, and no error — HMR read as enabled and did nothing. Setting `base` to the repo fixes it: verified through the real launcher with TWO successive reloads in one process (same pid, `v2` then `v3`), no restart. `--expose-internals` is NOT required — `cordis-plugin-loader` falls back to `node-addon-require-builtin`, so none of that flag's unstable-`internal/*` or authored-code-reaches-the-module-loader risk needs taking. A 2×2 over {base, flag} confirms base is decisive and the flag is irrelevant in both directions | — | done |
| **`profile.e2e` asserted `disabled: false` and called that "HMR hot"** — the row was enabled for as long as the watcher was pointed at the wrong tree, so the test passed throughout and docs/03 advertised a capability that never worked. Strengthened to also pin a cwd-derived `base`; `--dump-config` prints `!!js` unevaluated, so the resolved behaviour needs a running harness (`hmr-reload.e2e.test.ts`, env-gated) | the lesson generalizes: for a watcher/daemon, "configured on" and "doing anything" are different assertions, and only the second is worth a test | — |
| **The web surface still cannot hot-reload** — `dsh-web-app` disables `hmr` with an upstream TODO calling its reload lifecycle untested, and `dsh-headless` disables it too (a one-shot run has nothing to reload into). So rung 6 is proven on the app-server profile, NOT in the browser. Overriding a row whose owners call it untested is a deliberate non-default | our bundle layer could override it in an opt-in `openswarm-web-dev` profile now that the mechanism is understood and the base fix is known-good | when watching an agent self-modify in the browser is the goal |
| **Authored plugins are not sandboxed** — `swarm_author_plugin` hands in exactly one capability (`defineTool`), and the header comment's "no filesystem, no network, no process access is handed in" is literally true but reads as containment. It is not: a `data:` URL module cannot resolve BARE specifiers, but it can import `node:` builtins, and `node:fs` + `node:child_process` are both reachable from authored source (verified in-harness, pinned by `authoring.e2e.test.ts`). What actually contains an authored plugin is the approval gate (who reaches shared scope) plus whatever sandbox/permission layer the profile composes — which a bare hand-built context has none of. The scope policy is blast radius for MOUNTING, not a limit on what mounted code can do | comment and docs corrected; a real confinement story means composing the sandbox layer for authored modules, or a capability-restricted realm, and is a design pass rather than a patch | before authored plugins run untrusted-by-default |
...
| ~~Remote peer lifecycle~~ **done (2026-08-27)** — three layers. (1) DETECTION: a stream that ends unbidden fails pending turns loud (was a deadlock — the teardown that would have released them sat behind the same await), and an idle clock on the event stream, reset by ANY event so tool work keeps it alive, catches the child that is wedged rather than gone. (2) TASK RECOVERY: a failed claim is released and retried on a sibling, the presumed-dead member leaves the pool, and a task exceeding `maxTaskAttempts` (default 2) is abandoned along with everything transitively blocked by it, so a poison task cannot take the roster down one member at a time. (3) WARM RESTART: the dead member is respawned into its surviving worktree and briefed from a digest of its persisted log (`maxMemberRestarts`, default 1). What is still NOT recovered is true session continuity — see the resume row | — | done |
...
| ~~No real model has authored AND mounted a plugin~~ **done (2026-08-27)** — closed LIVE (Azure gpt-5.5, `openswarm web`, blank session). Asked to write itself a `swarm_uptime` tool at `lead` scope, the model authored the plugin source unaided, the approval gate rendered, `Allow once` granted, and it then CALLED its own new tool (`92.259643792`). 1 turn / 6 steps, 45.1K input / 659 output, 68% cache hit. The first attempt is the interesting part: the model's source omitted `output`, so the mount was refused with `plugin apply failed: Cannot read properties of undefined (reading 'render')` — it recovered on its own by grepping `defineTool\(` and reading `authoring.e2e.test.ts`, re-authored with `output.schema`/`output.render`, and mounted on the second approval. So agency is now proven end to end, INCLUDING recovery from a malformed first draft | — | done |
| **`swarm_author_plugin`'s `source` description under-specifies `defineTool`** — it says "call ctx.tools.register(defineTool({...}))" but not that `output` (with `schema` AND `render`) is mandatory, nor that `parameters` is a flat record rather than a JSON-Schema object. A live model wrote the plausible JSON-Schema shape, and the refusal it got back is an unhandled destructure (`reading 'render'`), not a validation message. Costs a wasted authoring round AND a wasted human approval — the gate fires before the source is ever compiled | one-line description fix plus, ideally, validating the tool shape BEFORE putting the mount to a human | next F3 touch |
| **Member sessions persist but never resume** — the member composition writes a full JSONL log per session, and `HarnessSdkJsonRpcServer.getOrCreateSession` consults an in-memory map for THAT process then falls through to `agents.create`, never to persistence. So a respawned child is amnesiac with its own history on disk beside it. We work around it by folding the log into briefing context (`recover.ts`), which recovers the narrative but not tool-call state or in-context nuance. Pinned by `member-resume.test.ts`, which flips the day upstream adds resume-on-miss | upstream `getOrCreateSession` consulting the store, or an `agents.create({ resume })` | file with the other upstream issues |
| ~~Cascade command gate graded the wrong tree~~ **fixed (2026-08-27)** — the confidence runner was built once in `dispatch` from `confidenceCwd ?? process.cwd()`, before any worktree exists, so under worktree execution it ran the verification commands against the REPO ROOT while the tiers edited a worktree. Since that root is the caller's own normally-green checkout, the gate returned 1 no matter what a tier did: a cascade could report `accepted` having changed nothing, which makes the whole command-gated self-modification story unsound. The runner now resolves the shared cascade worktree lazily per invocation (`CASCADE_TASK_KEY`, memoized by `SwarmGit`); an explicit `confidenceRunner` still wins. Pinned by `cascade-worktree.e2e.test.ts`, whose gate can only pass inside a worktree — it reports `[0, 0]` against the old code and `[0, 1]` against the new | — | done |
| ~~Agent-authored plugins do not survive a restart~~ **done (2026-08-27, rung 4)** — `persistDir` on the authoring row writes each APPROVED lead-scope plugin as `<name>.mjs` beside an `approved.json` manifest holding the SHA-256 of exactly what the human approved; `apply` restores them on boot and is now async, so the fiber is not ready until restored tools are (registration of `swarm_author_plugin` itself stays synchronous, before the first await). Restore is byte-pinned on purpose: member harnesses carry ordinary file-write tools, so without the digest a model could drop a `.mjs` into the directory and reach SHARED scope on the next boot having never faced the gate. Edited-after-approval and never-approved files are both skipped, and `self` scope is never persisted (no shared context to return to). OFF by default — persistence converts a one-time grant into a standing one, which is the operator's call. Four tests in `persistence.e2e.test.ts`, including the escalation attempt | — | done |
| ~~Rung 5: OpenSwarm edits its own source, gated by build+test, merged~~ **done (2026-08-28)** — closed LIVE (Azure gpt-5.5, `self-modify-live.test.ts`). A two-tier cascade was pointed at THIS repo with `confidence: { commands: ['npm ci', 'OPENSWARM_LIVE=0 npm run presubmit'], tau: 1 }`. Tier 1 made the edit in its worktree, the gate ran this repo's real build + typecheck + 102 tests INSIDE that worktree, scored 1, and the work merged to `swarm/<team>/integration`; our checkout was byte-identical afterward (HEAD and `status --porcelain` both asserted). 116s end to end, one tier. The change it made is itself the ledgered `swarm_author_plugin` description gap that a live model found the hard way in the rung-3 session — so the loop closed on itself: a model hit the defect, we ledgered it, and the swarm fixed it. Merged in `8bfaff7` | — | done |
| ~~`swarm_author_plugin`'s `source` description under-specifies `defineTool`~~ **done (2026-08-28)** — fixed by the rung-5 self-modification run, not by hand. The description now states that `output` is required with both `schema` and `render`, and that `parameters` is a flat record rather than a JSON-Schema object. NOT fixed: the refusal is still an unhandled destructure (`reading 'render'`) rather than a validation message, and the approval gate still fires BEFORE the source is compiled, so a malformed draft still burns a human approval | validating tool shape before prompting the human is the remaining half | next F3 touch |
| **The confidence gate inherits the driving process's environment** — `defaultConfidenceRunner` shells out with a `cwd` but no `env`. For a repo whose own suite is env-gated this bites hard: with `OPENSWARM_LIVE=1` set, the gate's `npm test` re-ran the live suite inside the task worktree — including the self-modification test driving the run, which then failed its own clean-checkout guard because the worktree is legitimately dirty. A CORRECT tier-1 edit scored 0. Cost a full tier and a real investigation to diagnose. Worked around at the call site (`OPENSWARM_LIVE=0` in the gate command), which is the right layer — a runner cannot guess what to scrub — but the trap is sharp and undocumented anywhere the caller looks | the call-site fix is in `self-modify-live.test.ts` and docs/03; a general fix would mean an `env` on the confidence config | if another gate hits it |
| ~~The pin that fixed the above was INERT~~ **fixed (2026-08-29)** — the fix shipped and documented as `confidencePinPaths: ['packages/*/tests']` pinned NOTHING. Git matches pathspec wildcards against WHOLE paths, so `packages/*/tests` never matches `packages/swarm/tests/recover.test.ts`; `git checkout HEAD -- 'packages/*/tests'` errors with "did not match any file(s)". `restoreFromBase` then treated "absent from base" as the legitimate "nothing may appear here" case and skipped silently — empty touched-list, no discard line, indistinguishable from a working pin. So the reward-hacking defence was decorative in EVERY live run of 2026-08-29, including the 16-cell matrix and the rung-5 test: members were free to edit the tests grading them. Found only by reading preserved diffs and noticing that pinned runs still contained `packages/*/tests/*.test.ts` edits — the numbers were all green. `gate-pinning.e2e.test.ts` missed it because every case used LITERAL paths; the documented glob form was never exercised. `restoreFromBase` now throws when a pathspec matches neither base nor worktree (a typo or a misunderstood glob, never a no-op), a test covers the inert case, and all callers compute literal directories. Note the members did not appear to exploit it — the test edits look like ordinary coverage added beside a feature | — | done |
| ~~The gate graded assets the graded party controlled~~ **fixed (2026-08-28, but see the row above — the fix was inert until 2026-08-29)** — a live reward-hacking path, not a hypothetical. The command gate runs in the member's own worktree, which is a full checkout INCLUDING `packages/*/tests`, so `npm run presubmit` read the suite from the tree it was grading: a member could pass by deleting a test instead of fixing the code. Observed, not inferred — the rung-6 `OPENSWARM_LIVE` recursion was this same mechanism pointed the other way, the gate executing a test file out of the worktree. `RunTeamOptions.confidencePinPaths` now restores given pathspecs from the base commit before EVERY gate run and cleans member-added files under them, so the pinned paths are exactly base state; edits there are discarded and never merged, and a discard is announced on `onProgress` rather than done silently. `gate-pinning.e2e.test.ts` asserts both halves — unpinned, the sabotage still succeeds and the gate reports a confident 1 on broken code (pinned in place so the vulnerability cannot regress unnoticed); pinned, the same member is caught. Residual: pinning is opt-in, and a run that legitimately adds tests must forgo it and accept a partly self-authored gate | — | done |
| ~~The self-modification gate has never been measured~~ **done (2026-08-28, run 2 `0e37033`)** — mutation-tested per [docs/04](04-gate-discrimination.md): containment 7/7, cascade logic 6/6, generic 4/5, and 0/4 semantics-preserving controls wrongly rejected. All four pre-registered thresholds met, so rung 5's admission test is a competent judge of the defects we could construct. Run 1 did NOT pass — a containment mutant disabling the pinning clean pass survived a test written for exactly that case, because that test asserted `confidence === 0` in a scenario where confidence is 0 whether or not the mechanism works. A tautological test is worse than none, since it is counted as coverage; it had been vouching for a security control since the hour it was written. Fixed, and the fix verified only after a false alarm of its own (the runner cuts from HEAD, so the uncommitted fix was invisible and the re-run reported the survivor again — it now refuses a dirty tree). What this does NOT license: the gate is proven against defects we thought to inject, not against the defects a model actually produces, which is what C1/C2's ungated arm measures | — | done |
| ~~Every worktree member run leaked its harness subprocess~~ **fixed (2026-08-29)** — `runMemberInSlot` awaited `started.result` and then disposed only the PROVIDER plugin fiber. Disposing the provider does not reap the run: `SubagentRun.dispose()` is what "cancels remaining work, reaches child quiescence, and releases resources", and we never called it. So every member run orphaned one `dsh-sdk-jsonrpc-demo` process, alive 10s later — well past the provider's 3s SIGTERM/SIGKILL and 6s EOF graces, so not slow teardown. The suite could not have caught it: vitest force-exits its workers, so orphans died with the runner. It also explains the repeated manual `pkill` this session, which I had been misattributing to runs I killed — they leaked on CLEAN completion too. Consequences: a long-lived `openswarm serve` accumulates zombie harnesses until the host runs out, and the planned eval matrix would have orphaned one process per cell. Found only because the eval adapter's boot check "timed out" — the cascade had SUCCEEDED and the process would not exit. Pinned by a test that reads the process table directly (verified to fail without the fix, `expected 1 to be +0`), since no in-process assertion can see it | — | done |
| **Remote peer path unchecked for the same leak** — `runRemotePeerTeam` spawns members through `RemotePeer`, a separate lifecycle with restarts and warm recovery, and has NOT been checked for the missing-dispose gap above. Flagging rather than assuming it is fine, since the same class of bug there would be invisible for the same reason | needs its own process-table assertion | before any long messaging run |
| ~~The gate did not gate the merge~~ **fixed (2026-08-28)** — `runTeam` ended with `finalize()` called unconditionally after `dispatch`, and `finalize` never consulted `accepted`, so a cascade that exhausted every tier without satisfying its gate merged its work into the integration branch anyway. The verdict decided escalation and the reported result, nothing about what landed — which makes "gated by build+test, merged" wrong as a description of rung 5. Hidden by luck so far: the live run's tier 1 passed. Found while designing C1/C2, because the planned gate-on/gate-off arms would have produced identical merges and measured nothing. `finalize({ merge })` now withholds on an unaccepted cascade: work is still auto-committed and the branch survives by name, but nothing folds into the integration branch, and `MergeOutcome.withheld` says so. Only the cascade has a whole-run notion of acceptance, so other topologies are unchanged | — | done |
| ~~A failing confidence gate reports a bare 0 with no diagnosis~~ **done (2026-08-28)** — the runner now returns `{ score, failedCommand, output }` (tail-capped at 4k, since a failing build's useful part is at the end), `CascadeAttempt.failure` records it, and the next tier's prompt carries the command and its output instead of "the verification commands did not pass". Pinned by a test that asserts the failure text reaches the MODEL, not merely the log — feedback that only lands in a log is not feedback. This also makes a rejected tier attributable, which C1/C2 needs: without it an environment problem and a genuine defect are indistinguishable, which is exactly how a correct edit scored 0 during the rung-5 run | — | done |
| **Rung 5 needs a worktree that can actually run the gate** — a git worktree is gitignore-clean, so `node_modules` is absent and `npm run presubmit` dies with `ERR_MODULE_NOT_FOUND`; the gate would score 0 for every tier regardless of the work. Symlinking the root `node_modules` in does NOT fix it: the workspace self-links (`openswarm-*`) resolve back to the ORIGINAL checkout, so `tsc` sees two identities of the same package and fails on `TS2717`/`TS2322`. So a self-modification gate on THIS repo must bootstrap the worktree hermetically (`npm ci &&  npm run presubmit`) rather than borrow the root's install | measured, not guessed — both failure modes reproduced 2026-08-27; the machinery is correct, this is the environment around it | before the first live self-edit run |
