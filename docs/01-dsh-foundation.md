...
| **`/swarm` as the FIRST action in a blank session renders nothing** — the command runs and its `command/run`/`command/done` pair lands in the session log, but the UI stays on the landing screen and never shows the result. Send any chat message first and it renders inline. Reproduced on both mock and live runs. NOT fixed by the job row: that renders in the conversation session header, which the landing screen does not show either | this is dsh's composer navigation for a blank session, not something the command can reach from the host side; an upstream fix or a `session.create`-then-navigate change owns it | upstream / composer |
