...
locked decisions) and [docs/archive/32-acp-implementation-plan.md](32-acp-implementation-plan.md) (Stage A,
...
`session/load`, [docs/archive/34](34-acp-b1-meta-swarm-plan.md)) and B2 (swarm-aware client + `swarm/steer`,
[docs/archive/35](35-acp-b2-rich-client-plan.md)) shipped on top; the §9 robustness pass + post-review caveats
...
- **B0.6 — Default flip + docs + live.** ✅ `acp` defaults to team (`--single` for Stage A); README
  "team over ACP"; committed live e2e (`OPENSWARM_ACP_LIVE`) for the coordinator turn, the permission
...
- [x] **Subprocess first-token latency.** Measured (deterministic bench,
      [acp-latency.bench.test.ts](../../test/integration/acp-latency.bench.test.ts), `OPENSWARM_ACP_BENCH=1`):
...
- [x] **Team `session/load` prose replay.** Done — the lead's prior narration is recovered from its
      ...
      arguments (live-only `tool_use_input`, not persisted) — low value, not planned ([docs/archive/34
      §8](34-acp-b1-meta-swarm-plan.md)).

**Rich-mode sub-stages (planned — see docs/archive/34 for the B1 scope):**
- [x] **B1 — `_meta.swarm` enrichment + capability negotiation + team `session/load`.** Shipped
      (B1.0–B1.4) — see [docs/archive/34-acp-b1-meta-swarm-plan.md](34-acp-b1-meta-swarm-plan.md). Enrichment
...
- [x] **B2 — `swarm/steer` ext + swarm-aware rich client.** Shipped — see
      [docs/archive/35-acp-b2-rich-client-plan.md](35-acp-b2-rich-client-plan.md). `swarm/steer` ext injects a
...
- [docs/archive/32-acp-implementation-plan.md](32-acp-implementation-plan.md) — Stage A (reused helpers + e2e harness)
