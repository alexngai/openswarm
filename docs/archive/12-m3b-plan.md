...
**Refines:** the six remaining items from §"Milestone M3 — orchestration depth + Claude Max subscription" in `docs/07-implementation-plan.md` that are not covered by `docs/archive/11-m3a-plan.md`.
...
1. `branch_lock.acquire("feature/x", { timeoutMs: 5000 })`: first caller succeeds (creates `.openswarm/branch-locks/feature-x.lock` atomically); second concurrent caller blocks; after first releases (via `finally`), second acquires; both eventually succeed.
...
20. `ask_user_question` timeout: worker sends IPC, parent never answers → after `OPENSWARM_ASK_TIMEOUT_MS` (default 10 min; test sets 100ms), worker receives `{ status: "timed-out" }`.
...
- Lock dir path: `<git-common-dir>/openswarm/branch-locks/` (inside `.git`, not the worktree root).
...
2.5. `scripts/branch-lock-report.ts` (new, optional CLI diag) — reads `.openswarm/branch-locks/*.lock` + takes a list of active tasks, runs `detectCollisions`, prints JSON. Not wired into the main CLI; invoked manually.
...
- Await response with `OPENSWARM_ASK_TIMEOUT_MS` (default `600_000`; override via env var `OPENSWARM_ASK_TIMEOUT_MS`).
...
- **Phase 3:** `npx vitest run src/engine/prompt-cache.test.ts src/engine/event-translator.test.ts` green; manual: run `openswarm prompt "hi"` twice with `/cost` in between; second run shows non-zero `cacheRead`.
...
- Prior milestones: `docs/archive/10-m2-plan.md` (UI depth + Tier 1 baseline; compaction observer; /cost + /status slash commands we extend), `docs/archive/11-m3a-plan.md` (BranchPolicy discriminated union we consume).
