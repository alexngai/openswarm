...
**Detailed Track B protocol report:** [docs/archive/26b-spike-track-b-codex-protocol.md](26b-spike-track-b-codex-protocol.md).
...
| `task_stop` (peer) | **WORKS** with `OPENSWARM_ALLOW_PEER_TASK_STOP=1` | Default ancestry-only policy needs to be revisited for flat peer teams. |
...
**Default peer-stop policy.** Today's default is ancestry-only — workers can only stop their descendants. The `OPENSWARM_ALLOW_PEER_TASK_STOP=1` env flag bypasses the check. For peer teams, peers stopping each other is reasonable (they're co-equal). Right answer is probably:
