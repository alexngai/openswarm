...
6. **Model alias table** (`src/providers/aliases.ts`) — read from `~/.openswarm/settings.json` under `aliases: { [alias: string]: modelId }`. Built-in defaults ship in code:
...
11. **Worker-entry + argv wiring** — `src/cli/worker-entry.ts` reads `--framework` + model; constructs the right engine. Swarm workers (M3a) propagate `OPENSWARM_FRAMEWORK` so subprocess workers inherit the parent's choice.
...
Rationale: M4a proves the end-to-end path. `OpenAIEnvAuth` just reads `process.env.OPENAI_API_KEY` and implements `headers()` / `isAuthenticated()` — enough for the engine to work. Matches the pattern of M0's `AnthropicApiKeyAuth` in spirit (simplicity). M4b adds `openswarm login --provider openai` with token persistence. If `OPENAI_API_KEY` is missing at engine construction, NativeEngine returns a friendly error with the env var name mentioned.
...
- `src/cli/worker-entry.ts` — reads `OPENSWARM_FRAMEWORK` env for inherited choice (strictly additive).
...
20. CLI `--framework auto` (default): `openswarm prompt --model gpt-4o "..."` selects NativeEngine + OpenAITransportProvider; `--model claude-sonnet-4-6` selects ClaudeAgentSdkEngine. Verified via `--dump-engine` debug flag (internal; not advertised in `--help`).
21. `openswarm --framework native --model claude-sonnet-4-6` exits non-zero with the error message:
    ```
...
23. Worker inheritance: parent invokes `openswarm swarm run --framework native tasks.jsonl` → child workers see `OPENSWARM_FRAMEWORK=native` → children instantiate NativeEngine (not SDK). Verified via subprocess integration test.
...
27. `scripts/smoke-m4a.sh` (live) covers: (L1) `openswarm --model gpt-4o prompt "say hi"` succeeds; (L2) `openswarm --model gpt-4o prompt "read README.md and summarize"` produces a tool-use → tool_result → text turn; (L3) `openswarm --model o3-mini prompt "..."` succeeds without errors from unsupported params (reasoning model quirk test); (L4) parallel tool execution on `--model gpt-4o prompt "read a.txt, b.txt, c.txt in parallel"` — 3 `tool_use` starts within 50 ms.
...
- No new dev deps.
- Record exact pinned versions in `docs/archive/13-m4a-plan.md` rev-2 after first successful install.
...
- `loadAliases(settingsPath?: string): Promise<AliasTable>` — read `~/.openswarm/settings.json`, extract `aliases` field; return `BUILTIN_ALIASES` merged with user aliases (user wins, emit `alias_shadowed` on collision).
...
AC: engine writes `native-snapshot.json` after the first turn completes; a second `openswarm --resume <session-id> --framework native` invocation reads it and materializes the message buffer without replaying the full JSONL.
...
- Read `OPENSWARM_FRAMEWORK` env var; default to `auto`.
- Thread through to engine construction identically to main.

6.4. `src/swarm/spawner.ts` (or wherever subprocess env is built — find via grep on `OPENSWARM_`): propagate `OPENSWARM_FRAMEWORK` from orchestrator to workers.
...
- **Live** (requires `OPENAI_API_KEY`):
  - [L1] `openswarm --framework native --model gpt-4o prompt "say hi"` succeeds.
  - [L2] `openswarm --framework native --model gpt-4o prompt "read README.md and summarize"` runs a tool turn.
  - [L3] `openswarm --framework native --model o3-mini prompt "..."` reasoning-model smoke (no param errors).
  - [L4] `openswarm --framework native --model gpt-4o prompt "read a.txt, b.txt, c.txt in parallel"` 3-tool parallel timing.

7.2. `scripts/smoke.sh --all` — extend to invoke `smoke-m4a.sh` alongside existing smoke scripts.

7.3. Docs:
- `docs/03-interfaces.md` — update `Provider` section: removes "stub" wording, documents `stream()` + `ProviderEvent` + `TransportProvider` marker. Add cross-ref to `docs/archive/13-m4a-plan.md`.
...
src/
  ...
  cli/
    ...
    worker-entry.ts                      # MODIFIED — read OPENSWARM_FRAMEWORK
scripts/
  smoke-m4a.sh                           # NEW
...
- **OpenAI model alias freshness.** Built-in aliases will go stale as OpenAI publishes new models. Document in README that users can override via `~/.openswarm/settings.json aliases`. Consider a `openswarm aliases list` CLI command — deferred to M4b.
...
- Prior milestones: `docs/archive/08-m0-plan.md` (AgentEngine + AuthSource foundation we extend), `docs/archive/10-m2-plan.md` (MCP client we reuse; compaction observer pattern we mirror), `docs/archive/11-m3a-plan.md` (role allowlists, BranchPolicy, subprocess spawn we inherit), `docs/archive/12-m3b-plan.md` (dispatchBatch, concurrencySafe, HookRuntime reentrancy we exercise for real).
...
Design: `docs/37-hardened-engine-design.md`. Implementation plan: `docs/archive/38-hardened-engine-implementation-plan.md`.
...
- **rev 3 (2026-05-27):** OpenAI prompt-cache deferred items (§Deferred bullet "OpenAI prompt caching", risk-table row "`@ai-sdk/openai` does not surface ... `prompt_cache_key`") are RESOLVED. Vercel SDK does expose `providerOptions.openai.promptCacheKey`. `ProviderRequest.sessionId` is the engine-agnostic carrier; openai-transport forwards it gated by per-model `ProviderCapabilities.promptCache` from the new `src/providers/capability-catalog.ts`. CLI uses `crypto.randomUUID()` per session (or the resumed id); subprocess workers use their `agentId`. See Q21 in `docs/06-open-questions.md` for the full decision record. No M4a scope changed; only the deferred-items list shortens.
