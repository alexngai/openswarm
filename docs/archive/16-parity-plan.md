...
Phased execution plan for closing the prioritized gaps in [15-parity-gaps.md](15-parity-gaps.md). Each phase has a goal, scope (by gap ID), acceptance criteria, and an estimate. Phases are sequenced by dependencies and risk, not strictly by priority — ship-ready work goes first so we're never blocked on a single decision.
...
**Goal:** Migrate openswarm from Node → Bun (required by `@opentui/core`'s `bun:ffi` native dependency) AND replace the Ink/React TUI substrate with OpenTUI/Solid. Decided 2026-04-22 per Q15 in [17-parity-design-questions.md](17-parity-design-questions.md) after Bun viability was confirmed empirically.

**Why both at once:** OpenTUI is Bun-only (uses `bun:ffi` to load a native Zig rendering library). Empirical probe showed openswarm's existing TypeScript runs cleanly under Bun with zero code changes — migration is tooling + distribution, not a rewrite. Doing the runtime swap alongside the TUI swap avoids two disruptive transitions.

**Gaps closed:** None directly — enabling work. Unlocks native solutions for T1, T2, T3, T4 (see [15-parity-gaps.md](15-parity-gaps.md)).
...
3. Update [package.json](package.json) scripts:
   ...
   - `build:compile`: `bun build --compile --target=bun-darwin-arm64 src/cli.ts --outfile=dist/openswarm` (standalone binary).
...
**Acceptance criteria:**
- `openswarm plugin install <local-path>` then `plugin enable <name>` surfaces the plugin's commands/hooks in the next session.
- `~/.claude/plugins/state.json` matches Claude Code's format (no drift) — diff against a pre-change snapshot.
- `openswarm --model grok-<x> "hello"` returns a response via xAI transport (local smoke).
...
**Goal:** Build the approval-prompt system for the first time. Claw parity: inline y/N prompt when mode denies a tool; no slash-command fallback. Design locked in [17-parity-design-questions.md](17-parity-design-questions.md) "Phase 2 — design lock (2026-04-22)".
...
2. **Persistent history** (T6)
   - Write submitted prompts to `~/.openswarm/history` (newline-delimited, 10k-entry cap).
...
**Acceptance criteria:**
- Running `openswarm --headless` with a deliberately destructive prompt is blocked at the validation layer (not just at exec time), with a structured error naming the submodule that rejected it.
...
> **Implementation note (2026-05-01):** Stages 3.0–3D of the App Server plan **shipped** as of this date. `CodexAppServerProvider` is live; `--framework codex-chatgpt` is wired end-to-end; 26 new tests pass. The SSE-based plan below is fully superseded. See [docs/archive/24-phase-6-codex-app-server-plan.md](24-phase-6-codex-app-server-plan.md) for the complete design.

> **Implementation note (2026-04-30):** REDESIGNED to use the **Codex App Server (JSON-RPC over stdio)** instead of the originally-planned custom Vercel AI SDK provider hitting `chatgpt.com/backend-api/codex/responses`. Web research confirmed the App Server is OpenAI's officially documented integration surface for third-party tools. The pivot eliminates the operator SSE spike (no longer needed) and changes the categorization from `TransportProvider` to `FrameworkProvider`. Full plan + design lock at [docs/archive/24-phase-6-codex-app-server-plan.md](24-phase-6-codex-app-server-plan.md). The scope/acceptance/estimate/blocker bullets below predate the pivot and are kept for historical record only.

**Scope (pre-pivot, see doc 24 for current):**
- ~~Finish M4b Phase 4 OAuth wiring.~~ Auth delegated entirely to `codex login`; openswarm owns zero OAuth code in this mode.
...
- ~~`openswarm login --provider openai`~~ → redirects to `codex login` instead.
- `openswarm --framework codex-chatgpt --model gpt-5 "..."` runs against ChatGPT subscription quota via the App Server.
