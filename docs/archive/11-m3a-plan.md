...
- Team roles (`src/swarm/roles.ts`): `Role = { name, systemPromptSuffix, allowedTools }`; built-ins `architect`, `executor`, `reviewer`; user-defined via `.openswarm/roles.json`
- Per-worker role wiring: env var `OPENSWARM_ROLE` → `RunConfig.systemPrompt` overlay + `RunConfig.allowedTools` filter
...
15. Custom roles: `.openswarm/roles.json` declaring a `{ name: "docs", systemPromptSuffix: "...", allowedTools: [...] }` is loaded at startup; registry-lookup returns it; unknown role name in `TaskPacket.role` → task fails at dispatch with `"unknown role: <name>"`.
...
17. Per-worker role wiring: a worker spawned with `role=reviewer` sees `process.env.OPENSWARM_ROLE === "reviewer"`; its `RunConfig.systemPrompt` contains the reviewer suffix; its `RunConfig.allowedTools` is exactly the reviewer allowlist; attempting `write_file` returns `permission: denied` (verified via hook fixture that logs `permission_denied` events).
...
- Subprocess spawner (Phase 6.5) propagates them via `OPENSWARM_ROLE` env var and an `allowedTools` env serialization.
...
2.2. `src/cli/swarm.ts` — replace the existing flat-string Zod schema with `TaskPacketSchema`. On parse failure, emit a migration hint to stderr: `"[openswarm] TaskPacket policies are now discriminated unions — see docs/archive/11-m3a-plan.md §Policy migration"`.

2.3. `docs/archive/11-m3a-plan.md` — add a "Policy migration" appendix with before/after examples. Users of existing `tasks.jsonl` files migrate:
...
- If worker-mode → check `ancestry.isAncestorOf(host.agentId, taskOwnerAgentId)`; reject with error if not ancestor (unless `OPENSWARM_ALLOW_PEER_TASK_STOP=1`).
...
```
Reads `.openswarm/roles.json` (Claude Code–style tiered discovery from M2 config-resolved). Validates via Zod schema mirroring `Role`. Unknown roles ignored with `degraded_role` lane event.
...
6.5. `src/swarm/subprocess-spawner.ts` — include `OPENSWARM_ROLE` env var in child spawn when set.

6.6. `src/cli/worker-entry.ts` — at startup:
- Read `process.env.OPENSWARM_ROLE`.
...
src/
  swarm/
    ...
    subprocess-spawner.ts      # MODIFIED — OPENSWARM_ROLE env var
    ipc/
  ...
  cli/
    ...
    worker-entry.ts            # MODIFIED — read OPENSWARM_ROLE → apply role to RunConfig
...
| Env var `OPENSWARM_ROLE` collides with user env | Low | Low | Prefix `OPENSWARM_` is already reserved by M1. No collision risk in practice. |
...
- **Dead-letter replay.** M3a writes dead-letter.jsonl but has no replay command. `openswarm swarm replay dead-letter.jsonl` is a one-liner we might add if operators ask; otherwise leave for M4.
- **Role trust boundary.** `.openswarm/roles.json` is user-controlled — same trust boundary as M2 hooks. Signed role manifests are M5+. Document in the custom-role loader JSDoc.
- **IPC additions require version tag.** Every new request method breaks backward compat between orchestrator and worker at different versions. Add a `OPENSWARM_IPC_VERSION` handshake? Not for M3a (single repo, single version shipped together); revisit at M4.
...
- Prior milestones: `docs/archive/09-m1-plan.md` (swarm — landed), `docs/archive/10-m2-plan.md` (UI depth — landed at `3b17fbd`)
...
- **rev 3 (2026-04-21):** execution complete. All 7 phases shipped on `mvp` (0+1 commit `327bebd` through Phase 7 commit `<TBD>`). See m3a-complete tag.
