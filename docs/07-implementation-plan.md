...
1. API-key path: `ANTHROPIC_API_KEY=... openswarm prompt "add a docstring to file.ts"` completes a real multi-turn tool-using session and writes the session log.
2. Subscription path: `openswarm login` → OAuth flow → `openswarm prompt "..."` runs against the user's Claude Max subscription.

`openswarm --headless --task-file=t.json` produces a parseable event stream on stdout in both paths.
...
**Exit criterion:** `openswarm swarm run tasks.jsonl --concurrency 3` spawns 3 subprocess workers on 10 tasks, each task runs to completion or failure with isolated session logs, and `results.jsonl` is machine-parseable.
...
- Login UX: `openswarm login` runs the Agent SDK's OAuth flow and persists tokens to `~/.openswarm/auth.json`.
...
- `openswarm login --provider codex-chatgpt` runs the OAuth PKCE flow end-to-end.
...
14. `src/cli/init.ts` — scaffolding (`.openswarm/`, `.gitignore`, `CLAUDE.md`)
