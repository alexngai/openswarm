...
Key architectural signals for openswarm:
...
Notes: the Rust trait returns `Vec<AssistantEvent>` rather than an async stream. In TypeScript, the equivalent would be `AsyncIterable<StreamEvent>`, matching openswarm's `docs/03-interfaces.md`. `ToolUse.input` is carried as a JSON string, not a parsed value.
...
- **Per-worktree isolation**: `SessionStore` that namespaces by a deterministic 16-char hex fingerprint of the workspace root. This is the claw response to "phantom completions" in parallel lanes — directly relevant to openswarm where multiple workers share a parent process.
...
- **Session tracer vs. event stream**: claw pipes runtime traces through a `SessionTracer` trait backed by a telemetry sink. openswarm already has an event stream as the primary observability surface — is a separate tracer abstraction warranted, or can we fold trace-worthy events into the existing stream?
...
- **Provider capabilities**: claw has no `ProviderCapabilities` concept — the loop assumes streaming + tool use + prompt cache events. Our `Provider` interface has `capabilities`, which the loop should actually check. This is a openswarm improvement over claw, not a port — flag it as a net-new thing to design.
...
- **Ask rule vs hook-Allow precedence**: claw explicitly tests that hook `Allow` *still respects* ask rules (prompt is required). This is subtle — confirm the openswarm behavior spec matches before copying blindly.
...
- **Session health probe tool name is hardcoded to `glob_search`**. openswarm's tool naming in `tools/tier0/` may differ; either standardize on `glob_search` or make the probe tool configurable.
- **Load-bearing for multi-agent swarm**: The "phantom completions" pattern (ROADMAP #41) is the biggest design lesson here — multiple serve instances writing to shared session storage while reporting success against different worktrees. openswarm's orchestrator-spawns-workers model has the same failure mode. The SessionStore + workspace_root validation is the answer. **Treat this as non-negotiable for v0.**
