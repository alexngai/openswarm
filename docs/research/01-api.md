...
- [skip] `from_env_or_saved` that ignores saved OAuth when env is absent — surprising behavior; openswarm should make env-vs-saved precedence explicit at call sites.
...
5. **OpenAI-compat stream usage for xAI/DashScope** — `should_request_stream_usage` only returns true for OpenAI. Does this mean streaming from xAI and DashScope never reports token usage? If so, openswarm should document that limitation explicitly for downstream cost accounting.
...
8. **`CLAW_CONFIG_HOME` vs `CLAUDE_CONFIG_HOME`** — claw-code uses different env vars for OAuth credentials vs. prompt cache (`.claw/` vs `.claude/`). openswarm needs to pick one canonical root (proposed: `$XDG_CONFIG_HOME/openswarm/`).
...
10. **`AuthSource::from_env_or_saved` ignoring saved OAuth** — the current behavior (test at `anthropic.rs:1157-1177`) feels buggy but is intentional. Confirm which precedence openswarm wants.
11. **Model registry table** — claw-code hard-codes model IDs and limits in source. Should openswarm make this loadable from a config file, or keep it in code for lock-down-via-tests?
