...
Slice extracted for openswarm's orchestrator / `SwarmHost` design. Source: `references/claw-code/rust/crates/runtime/src/*` plus the Tier 2/3 tool sections of `crates/tools/src/lib.rs`.
...
Key implication for openswarm: keep claw-code's **data models** (TaskPacket, lane event names, Worker lifecycle states, BranchLock collision detection) but replace the **transport** (threads + in-memory registries + screen-scraping) with subprocess JSONL and a `SwarmHost` interface that allows future swap to a real bus.
...
`TeamCreate` accepts a `name` and an array of **task objects** (not task IDs) but `run_team_create` only extracts `task_id` fields — so the tool as written requires you to have already called `TaskCreate`. No roles, no member-to-role mapping, no system-prompt overlays. This is a stub for openswarm's concept of "team roles."
...
### 4.5 Relevance to openswarm

Almost none of this maps directly. openswarm's worker is a subprocess we own, talking JSONL over stdio — we do not need to parse terminal text. What *is* reusable:
...
`prompt_delivery | trust_gate | branch_divergence | compile | test | plugin_startup | mcp_startup | mcp_handshake | gateway_routing | tool_runtime | workspace_mismatch | infra`. This is a directly-borrowable vocabulary; openswarm should adopt it almost verbatim in our `events/lane.ts`.
...
**Bottom line on "remote":** claw-code has no real remote-agent transport. It has a webhook hammer and an ops-time HTTP proxy. openswarm's `remote_trigger` spec can be exactly this — a typed HTTP webhook. Real remote-agent invocation is a future design.
...
### 8.5 Critical caveat
The Agent tool runs sub-agents as **in-process threads sharing API client state**. This is wrong for openswarm's thesis ("one agent is a tool, N coordinated agents is the product"). openswarm must use real subprocesses for the isolation property — a worker crash must not take down the parent.
...
This is the biggest gap vs. what the 04-tool-tiers.md doc describes (`send_message` + `check_inbox` by agentId). openswarm designs this fresh.

## 10. Requirements for openswarm
...
- [v0] **agentId convention** stable for an agent's lifetime; passed via env (`OPENSWARM_AGENT_ID` per 05-swarm-model.md) and present on every lane event. Ours, not claw-code's.
- [v0] **Atomic state file** at `{cwd}/.openswarm/worker-state.json` (tmp + rename) — this is claw-code's observability pattern and is cheap to adopt.
...
2. **Where does the task registry live across processes?** Options: (a) parent-owned, workers RPC in via JSONL requests; (b) file-backed under `.openswarm/tasks/*.json`; (c) no cross-process task registry in v0 — only `agent_id` + lane events. claw-code punts (single-process only); we must decide.
