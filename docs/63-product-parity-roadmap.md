...
Ten are delivered: `WP-00`, `WP-00a`, `WP-01`, `WP-02`, `WP-03`, `WP-04`, `WP-05`, `WP-06`, `WP-07`, and `WP-09`. Every R1 package is closed, so the remaining R1 exit conditions are review items rather than implementation: the walking skeleton and revised loading model need approving, and the `DDP-*` outcomes need certifying on Linux x64. The ready queue is now entirely R2, headed by `WP-08` — automatic multi-turn and crash resume, which is also what gives `FileEventStore` its first production caller and the importer a subcommand worth having. `bun scripts/parity-ready.ts` derives this from the manifest rather than from this paragraph.
