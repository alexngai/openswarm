...
Execution companion to [docs/30-acp-compatibility-plan.md](30-acp-compatibility-plan.md) (Stage A
...
| Integration | a scripted in-process ACP **client** (use the SDK's `ClientSideConnection` over an in-memory duplex) drives initialize→new→prompt→cancel against a `ScriptedTestEngine`; assert the frame sequence | `src/acp/agent.test.ts` |
| Smoke | `scripts/smoke-acp.sh` pipes a recorded transcript to the real binary; asserts stdout is pure JSON-RPC | `scripts/smoke-acp.sh` |
| E2E | real Zed `agent_servers` entry → manual checklist (doc 30 §7 acceptance) | manual |
...
## Key references
- [docs/30-acp-compatibility-plan.md](30-acp-compatibility-plan.md) (Stage A design + acceptance)
