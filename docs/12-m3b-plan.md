...
1. `branch_lock.acquire("feature/x", { timeoutMs: 5000 })`: first caller succeeds (creates `.swarm-coder/branch-locks/feature-x.lock` atomically); second concurrent caller blocks; after first releases (via `finally`), second acquires; both eventually succeed.
...
20. `ask_user_question` timeout: worker sends IPC, parent never answers → after `SWARM_CODER_ASK_TIMEOUT_MS` (default 10 min; test sets 100ms), worker receives `{ status: "timed-out" }`.
...
- Lock dir path: `<git-common-dir>/swarm-coder/branch-locks/` (inside `.git`, not the worktree root).
...
2.5. `scripts/branch-lock-report.ts` (new, optional CLI diag) — reads `.swarm-coder/branch-locks/*.lock` + takes a list of active tasks, runs `detectCollisions`, prints JSON. Not wired into the main CLI; invoked manually.
...
- Await response with `SWARM_CODER_ASK_TIMEOUT_MS` (default `600_000`; override via env var `SWARM_CODER_ASK_TIMEOUT_MS`).
...
- **Phase 3:** `npx vitest run src/engine/prompt-cache.test.ts src/engine/event-translator.test.ts` green; manual: run `swarm-coder prompt "hi"` twice with `/cost` in between; second run shows non-zero `cacheRead`.
