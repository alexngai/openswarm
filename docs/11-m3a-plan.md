...
2.2. `src/cli/swarm.ts` — replace the existing flat-string Zod schema with `TaskPacketSchema`. On parse failure, emit a migration hint to stderr: `"[openswarm] TaskPacket policies are now discriminated unions — see docs/11-m3a-plan.md §Policy migration"`.
