...
**Decision:** Keep `openswarm`. Rename remains cheap pre-v0.1; no compelling pull toward an alternative surfaced during review.
...
- **CLI (single-agent path):** `crypto.randomUUID()` for fresh sessions, the resumed id for `--resume`. Same id used for all turns in the loop.
- **Subprocess workers:** the worker's `agentId` (already a randomUUID generated by the host). Each worker gets its own stable key so routing stays per-worker — siblings don't hot-spot a single backend.

**Decision log entry:**

| Date | Question | Decision | Rationale |
|---|---|---|---|
| 2026-05-27 | Q21 OpenAI prompt cache | Wire `sessionId → providerOptions.openai.promptCacheKey`; capability-gated by per-model catalog; per-worker keys for fan-out | Vercel SDK exposes the surface; keeps server cache warm across multi-turn sessions; per-worker keys avoid hot-spotting |

## Q22. Wire-protocol versioning for JSONL streams — RESOLVED

**Question:** Q7 (LaneEvent additive) settled the event-shape question, but offered no story for "what protocol version is this file?" — JSONL consumers had no way to detect future breaking changes.

**Resolution:** A wire-level metadata event was introduced (`src/swarm/wire-protocol.ts`) — orthogonal to LaneEvent. The first line of any persistent JSONL wire is a `{type: "_metadata", protocol_version, created_at, producer}` record stamped on first open. A `WireMigration` registry exists for chaining future shape transforms (`resolveWireMigrations(fromVersion)`); the registry is empty at v1.0. Reading uses `parseWireLineSafe` and `iterateWireLines` which drop a truncated trailing line (crash tolerance) and yield `null` on per-line parse errors rather than throwing.

**LaneEvent classification.** `isLiveOnlyLaneEvent(type)` partitions the event surface into "live-only" (high-rate deltas: `text_delta`, `tool_use_input`, `heartbeat`) and "recorded" (everything else, including `worker_lifecycle_changed` — that's a typed lifecycle record, not a streaming delta). The `team-daemon` filters by `isRecordedLaneEvent` before writing `events.jsonl`, dropping live-only events from disk while leaving in-process subscribers untouched.

**Decision log entry:**

| Date | Question | Decision | Rationale |
|---|---|---|---|
| 2026-05-27 | Q22 wire versioning | Wire-level metadata event + migration registry, orthogonal to Q7's additive LaneEvent convention | Q7 covers event-shape additivity; Q22 covers file-level version detection for future breaking changes |

## Q23. Ctrl-S "steering" — submit follow-up mid-turn — RESOLVED

**Question:** How does a user push a follow-up message to the model without aborting an in-flight turn?

**Resolution:** New reducer event `{type: "steer"; text: string}` valid only from `streaming` state. In `app.tsx`'s `handleKey`, Ctrl-S with a non-empty buffer dispatches `steer` (visual feedback: a `(steered) <text>` user transcript entry; input buffer cleared) AND calls `props.onSubmit?.(text)` — the existing `runRepl` prompt queue picks it up at the next turn boundary. Simpler than kimi-code's mid-step injection (which would require engine-side plumbing) but delivers the same UX win: the user can type during streaming without losing input or aborting.

**Decision log entry:**

| Date | Question | Decision | Rationale |
|---|---|---|---|
| 2026-05-27 | Q23 Ctrl-S steering | Queue for next turn via existing onSubmit → prompt queue path | Zero engine plumbing; matches kimi-code's UX without invasive changes; can upgrade to mid-step injection later if needed |
