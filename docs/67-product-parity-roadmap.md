...
- **Path canonicalization on macOS, which is not the same job as isolation.** This package was scoped as containment and packaging, on the assumption that what differs off Linux is the *enforcement* primitive. A macOS trust-store defect found while working on `WP-00a` says otherwise: `src/trust/store.ts` keys grants with `path.resolve`, `src/trust/provenance.ts` canonicalizes with `fs.realpath`, and because `/tmp` is itself a symlink to `/private/tmp` on macOS, a grant written under one spelling is invisible to a lookup under the other. `FX-TRUST-006` fails there today.

  Three things about it are worth recording rather than just the fix. It fails *closed* — the grant is not found, so trust is withheld — which makes it a usability defect and not a bypass; both spellings name the same file, so there is no path by which it grants what it should not. It is out of every current package's scope and legitimately so: `WP-02` declares `linux-x64` only, and every macOS and Windows cell in the manifest belongs to this package, so nothing was overstated. And it was found by accident, which is the part that should worry us — nobody was looking, and a defect of this shape is invisible until someone runs the suite on a Mac.

  The correction is to the scope rather than the schedule: what is unproven off Linux is every assumption about how paths resolve, not only whether an escape can be prevented. `WP-03`'s canonicalization work was gated on `linux-x64`, so its guarantees are Linux guarantees. The six person-weeks here were estimated against isolation and packaging and do not include auditing platform-dependent path assumptions across the trust store, the workspace authority, and the tool path; that is a re-estimate this package needs before R5 planning, not a number to invent here.
