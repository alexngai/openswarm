...
The package the re-estimate recommends. Gate: `./scripts/verify-parity-wp.sh WP-00a <cell>`, `A1`–`A13` passing. Fixtures `FX-ESCAPE-001` (containment corpus, including the symlinked-parent case), `FX-GATE-001` (every engine gates tool calls), and `FX-AUDIT-001..019` (the audit journal is durable with no storage configuration, holds no message text, survives `SIGKILL`, pairs a pre-decision fact with a terminal one for every side effect on both dispatch paths, and reconciles what a real crash leaves dangling when the session next starts). Delivered in well under the 4 person-weeks estimated, for a reason worth carrying forward: the estimate assumed the chokepoints had to be collapsed, and they did not.
