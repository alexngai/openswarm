...
A third is open and deliberately unfixed here. The `baseline` cell on `macos-arm64` is red: `test/integration/open-weight-repair.e2e.test.ts`, "dispatches the call and continues the run", makes four model round trips on macOS where it makes two on Linux and expects two. It predates this work — it fails with these changes stashed — and it is not a platform *quirk* in the test, since a doubled dispatch count in the open-weight repair path is either a retry firing that should not or a real difference in how the run continues. It has no fixture ID and no package: the repair path is verified by the baseline suite rather than by a gate, which is why nothing owned it. That needs an owner before the macOS baseline can be claimed green, and it should not be claimed green until then. 4787 of 4810 tests pass on macOS; one fails and 22 skip.
