...
The package the re-estimate recommends. Gate: `./scripts/verify-parity-wp.sh WP-00a <cell>`, `A1`–`A12` passing. Fixtures `FX-ESCAPE-001` (containment corpus, including the symlinked-parent case), `FX-GATE-001` (every engine gates tool calls), and `FX-AUDIT-001..014` (the audit journal is durable with no storage configuration, holds no message text, survives `SIGKILL`, and pairs a pre-decision fact with a terminal one for every side effect on both dispatch paths). Delivered in well under the 4 person-weeks estimated, for a reason worth carrying forward: the estimate assumed the chokepoints had to be collapsed, and they did not.
