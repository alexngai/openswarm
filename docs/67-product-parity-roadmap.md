...
The package the re-estimate recommends. Gate: `./scripts/verify-parity-wp.sh WP-00a <cell>`, `A1`–`A11` passing. Fixtures `FX-ESCAPE-001` (containment corpus, including the symlinked-parent case), `FX-GATE-001` (every engine gates tool calls), and `FX-AUDIT-001..011` (the audit journal is durable with no storage configuration, holds no message text, survives `SIGKILL`, and pairs a pre-decision fact with a terminal one for every side effect). Delivered in well under the 4 person-weeks estimated, for a reason worth carrying forward: the estimate assumed the chokepoints had to be collapsed, and they did not.
...
What is still owed here, stated plainly. `EffectRuntime.recover()` contains the reconciliation logic and still has no caller, so a crash now leaves a correct dangling `AttemptPrepared` that nothing reads — recovery is recorded but not yet performed. The batch dispatch path does not go through the ledger, so a batched call is prepared and never resolved. The kernel's weaker staging is still present rather than retired in favour of the contained write. And the end-to-end evidence is a gate run rather than a live one; the fixtures drive the real gate and the real ledger, but no live run has yet been observed writing correlated records.
