...
- [64-complementary-specialist-routing.md](./64-complementary-specialist-routing.md) — H3 design: test whether DIVERSITY (complementary specialists, not a cost ladder) improves accuracy + efficiency. Adds the **complementarity pre-check** (κ = mutual-exclusive coverage; κ≈0 ⇒ routing can't raise accuracy) and an N-way oracle/real-signal router over the docs/62 offline method; diverse Bedrock families (Qwen-Coder/DeepSeek/Devstral/GLM), cheap single-shot κ-screen first, then agentic (proposal; extends 50/62)
...
- [65-live-eval-handoff.md](./65-live-eval-handoff.md) — runbook to finish the one doc-55 measurement that needs a real model: the TE-25 constraint-retention eval (baseline vs section vs verbatim), with the decision rule and where to record results
...
- [63-tool-call-repair.md](./63-tool-call-repair.md) — engine-level repair and recovery of malformed tool calls from open-weight models behind OpenAI-compatible endpoints: name/argument/envelope repair, rebuilding calls the provider dropped mid-stream, and extracting calls the serving layer emitted as text (Hermes/Llama/Mistral/DeepSeek syntaxes) — fixes the silent one-turn stop when vLLM has no `--tool-call-parser`; default-on, `OPENSWARM_TOOL_CALL_REPAIR`. Also closes the adjacent open-weight gaps: F1 context-window probing (`/v1/models` `max_model_len`, LiteLLM `/model/info`) replacing the hardcoded 200k guess that broke compaction sizing, F2 `toolChoice` plumbed to every transport, F3 sampling params (`--temperature`/`--top-p`/`--top-k`) reachable from `RunConfig`, F5 one-shot `toolChoice: required` escalation when a turn stops with unrecovered tool-call syntax. Verified by a hermetic e2e that drives the compiled CLI against a real OpenAI-compatible SSE server (implemented; F4 eval + F6–F9 open, extends 62)
- [66-live-eval-handoff.md](./66-live-eval-handoff.md) — runbook to finish the one docs/63 measurement that needs a real open-weight tier (F4): repair-on vs repair-off resolve rate, with the server-configuration confound (`parser-on`/`parser-off`) as a first-class axis, a free pre-check that kills the cell before any spend if repair never fires, and the decision rule. Also re-opens whether docs/62's "small tier solves ~nothing uniquely" was partly measuring dropped tool calls (not run)
