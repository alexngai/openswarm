...
- **TE-25 discriminating re-run (new, open):** the instrument saturated at baseline 100% across the **full matrix tested** — {default, hard} fixtures × {default effort, `reasoning_effort=none`}, 180 runs, zero drops. We've now ruled out **two** levers: reasoning effort (effort=none = non-reasoning gpt-5.5) and fixture difficulty (the `hard` set: in-passing, multiple-per-fixture, paraphrasable identifiers). gpt-5.5 keeps even paraphrasable identifiers verbatim under the byte-exact baseline, so the section/verbatim additions aren't *needed* on this model and we still can't measure their *value*. **The only untested discriminator left is a genuinely smaller/weaker summarizer** — `gpt-4.1` (deployment already live on the Azure resource) or a small open model via `litellm/…`. If baseline drops there while section/verbatim hold, that's the evidence justifying TE-25a's default and possibly flipping TE-25b; if it too saturates, close TE-25 as "cheap insurance, unmeasurable on frontier models." Low priority — the additions are cheap and already shipped; this only affects whether we can *defend* them with data.
