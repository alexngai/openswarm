...
| F10 | **Web search (configurable backend)** | ✅ | P3 | M | `tools/tier1/web_search.ts`: Pluggable `SearchBackend` interface with default DuckDuckGo HTML scraping backend (matches Codex `web-search/`). `hostMatchesList()` for subdomain-aware domain filtering. `allowed_domains`/`blocked_domains` input params. DDG redirect URL decoding, deduplication, 8-hit cap. `SWARM_WEB_SEARCH_BASE_URL` env override (matches Codex `CLAWD_WEB_SEARCH_BASE_URL`). Network policy enforcement for search host. Result formatting with ranked titles/URLs/snippets. Backend registry: `getSearchBackend()`/`setSearchBackend()`/`resetSearchBackend()`. OpenPage/FindInPage covered by existing `web_fetch`. |
...
- **2026-06-06** — F10: Web search with configurable backend. Pluggable `SearchBackend` interface with DuckDuckGo HTML scraping default (matches Codex `web-search/`). Domain filtering (`allowed_domains`/`blocked_domains`) with subdomain-aware matching. DDG redirect decoding, deduplication, 8-hit cap. `SWARM_WEB_SEARCH_BASE_URL` env override. Network policy integration. OpenPage/FindInPage covered by `web_fetch`. 27 new tests passing. Index test updated (4→6 tools).
