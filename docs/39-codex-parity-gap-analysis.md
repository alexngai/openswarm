...
| F10 | **Web search + browsing (full Codex parity)** | ✅ | P3 | M | `web_search.ts`: Pluggable `SearchBackend` interface, DuckDuckGo HTML scraping default, batch `queries` support (up to 5 with cross-query deduplication), domain filtering, 8-hit cap, `SWARM_WEB_SEARCH_BASE_URL` env override. `web_fetch.ts`: HTTP→HTTPS auto-upgrade (matches Codex — exempts localhost/127.0.0.1/::1), prompt-driven extraction (`"title"` → `<title>` tag, `"summary"`/`"summarize"` → 900-char preview), `find` param for in-page substring search with context lines (replaces Codex `find_in_page` action). Full Codex web-search parity across two composable tools. |
