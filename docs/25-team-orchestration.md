...
- **Mixed-engine consultant pattern** (V0.4.Q9) — `agent({framework: "..."})` schema is wired in 4E.1; framework propagation through `StandaloneHost.spawn` → `spawnWorker` was silently broken until 4M.5 fixed it (with regression tests in `standalone-host.test.ts`). The wiring is now correct end-to-end at the unit level. The Q9 live cross-engine smoke ships in 4M.8 as `scripts/smoke-codex-consultant.sh` (operator-run; gated on `codex` CLI presence + ChatGPT Plus/Pro auth). **Live finding (2026-05-03):** the consultant smoke runs the full chain (Claude transport → agent tool → IPC spawn → codex subprocess) but the spawned codex subprocess currently fails with an empty error. The codex peer-team smoke (`scripts/smoke-codex-team.sh`) succeeds, so the defect is specific to the worker-spawned-codex path enabled by 4M.6 — the IPC-handler unit tests didn't catch it because they mock `spawnFn` one level above the actual subprocess failure. Tracked as a 4M.9 follow-up; CI continues to use the offline captured-trace test for the underlying protocol contract.
