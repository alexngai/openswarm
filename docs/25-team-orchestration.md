...
| 6C | Federation prep — **shipped**. New `list_agents` Tier 2 tool returns the inbox backend's persistent agent registry in the caller's scope (distinct from `team_members`, which shows the live peer roster). `AgentInboxBackend.enqueue` now `putAgent`-registers both sender and recipient implicitly so `listAgents` works without bootstrap. `send_message.to` parses `agent@system` / `@system` federation addresses via agent-inbox's official `parseAddress`; remote addresses surface a structured "parsed but not yet routed; planned for v0.8+ MAP federation" error rather than failing as "unknown_recipient". New IPC method `message.list_agents`. |
