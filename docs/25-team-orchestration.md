...
- **Mixed-engine consultant pattern** (V0.4.Q9) — `agent({framework: "..."})` schema is wired in 4E.1; framework propagation through `StandaloneHost.spawn` → `spawnWorker` was silently broken until 4M.5 fixed it (with regression tests in `standalone-host.test.ts`). The wiring is now correct end-to-end at the unit level. A live cross-engine smoke (Claude transport member calling a Codex consultant) is still not in tree — verify against your engine creds before relying on it in production.
