...
- **Mixed-engine consultant pattern** (V0.4.Q9) — `agent({framework: "..."})` schema is wired in 4E.1; framework propagation through `StandaloneHost.spawn` → `spawnWorker` was silently broken until 4M.5 fixed it (with regression tests in `standalone-host.test.ts`). The wiring is now correct end-to-end at the unit level. The Q9 live cross-engine smoke ships in 4M.8 as `scripts/smoke-codex-consultant.sh` (operator-run; gated on `codex` CLI presence + ChatGPT Plus/Pro auth). CI continues to use the offline captured-trace test for the same protocol contract.
