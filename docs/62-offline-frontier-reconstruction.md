...
**Status:** methodology + Phase 0/1/1.5/2 results. First positive frontier expansion on a clean known-params pair, replicated across HumanEval + MBPP (§4.2, F5–F11); Phase 2 (§5.2, F12–F14) shows the oracle pre-check **transfers to agentic SWE** (Qwen-Coder 3B/35B, FAIL→PASS vs the dead haiku/Nova pairs) — a robust structural positive across hard-18 / easy-16 / combined-34 slices (oracle Pareto-dominates mono-large on FLOPs everywhere; ~1.3× magnitude, oracle ceiling not yet a deployable signal). Extends 50 (the thesis), 54/59/60/61 (the live-experiment line that reached "H2.1 not robustly supported").
...
**Still open after Phase 2:** (a) extract the SWE repro/compile-gate as the deployable signal — turn the oracle ceiling into a real cascade (currently `sig_visible=null` ⇒ oracle-only); (b) ≥3 seeds (all slices are 1-seed); (c) the offline cold-E cost excludes live handoff bloat (docs/61 F3), so a live cascade would cost more — bound it; (d) the ~1.3× agentic FLOPs magnitude is modest vs single-shot — a stronger/cheaper small tier or better routing could widen it. The transfer is now a robust structural positive; the deployable-signal and seed-count work would make it decisive.
