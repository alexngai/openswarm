/**
 * M4a Phase 6 integration test — OPENSWARM_FRAMEWORK env var propagation.
 *
 * Verifies that a worker subprocess launched with OPENSWARM_FRAMEWORK=native
...
describe("framework inheritance via OPENSWARM_FRAMEWORK", () => {
  it("OPENSWARM_FRAMEWORK=native --dump-engine --model gpt-4o → engineId:native", () => {
    const result = spawnSync(process.execPath, [CLI, "--model", "gpt-4o", "--dump-engine", "say hi"], {
      env: {
        ...process.env,
        OPENSWARM_FRAMEWORK: "native",
  ...
  it("OPENSWARM_FRAMEWORK=auto --dump-engine --model claude-sonnet-4-6 → engineId:claude-agent-sdk", () => {
    const result = spawnSync(process.execPath, [CLI, "--model", "claude-sonnet-4-6", "--dump-engine", "say hi"], {
      env: {
        ...process.env,
        OPENSWARM_FRAMEWORK: "auto",
