 ...
 * scripted engine enabled via OPENSWARM_TEST_SCRIPT. No live API calls.
 *
...
//
// Points OPENSWARM_PLUGINS_DIR at the fixture tree and asks the CLI to
...
// OPENSWARM_CONFIG_DIR to its parent, and asserts that get_time + echo
...
describe("E: hook config resolution emits startup log", () => {
  it("logs 'hooks loaded from' when .openswarm/hooks.json is present", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "swc-hooks-"));
    try {
      const hooksDir = path.join(tmp, ".openswarm");
