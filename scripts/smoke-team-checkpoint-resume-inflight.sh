...
# with a verify-then-continue preamble AND the worker resumes its prior engine
# session (per-unit sidecar).
#
...
#
# Flow: 1-member fanout. Worker emits a line (its engine session id is persisted
# to the per-unit sidecar immediately), then waits. We wait until the unit is
# recorded in-flight AND its sidecar exists, kill -9 the daemon mid-flight,
...
if ! echo "$RUN2_EVENTS" | grep -q "re-dispatching (in-flight at crash"; then
  echo "[fail] run 2 did not emit the in-flight re-dispatch note"
...
echo
echo "RESULT: PASS"
echo "  Team crash-recovery T2 verified live: a member crashed mid-flight was"
echo "  re-dispatched with a verify-then-continue preamble and resumed its"
echo "  prior engine session from the per-unit sidecar."
exit 0
