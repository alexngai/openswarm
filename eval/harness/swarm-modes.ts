 ...
 * `openswarm "<prompt>"` headless run is single-agent by construction. The TEAM arm simply adds
 * openswarm's spawn + coordination tools via `scaffold.extraTools`, letting the agent fan out
 ...
 *
 * ⚠ LIVE-VALIDATE: confirm that a headless `openswarm "<prompt>"` with these tools present actually
...
/** openswarm teammate-spawn + coordination tools (from src/tools, src/swarm). */
