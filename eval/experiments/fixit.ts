 ...
 * cell in a cloud sandbox built from ONE generic template (node + the packed local openswarm +
 ...
 *   - single : one openswarm agent (the generic CLI adapter).
 ...
 *   RUN_FIXIT=1 [AWS creds + openswarm on PATH] npx tsx eval/experiments/fixit.ts   # real (2b)
...
// (gpt-5.5 via the azureoai/ DIRECT transport = openswarm's NATIVE engine, no Claude SDK).
...
/**
 * Bedrock-direct auth for the in-process agent (docs/45: sandbox auth = Bedrock). openswarm routes
...
/** Azure-direct auth — the `azureoai/` transport calls Azure OpenAI directly, using openswarm's
...
// node + the LOCAL packed openswarm (so the working-tree native-engine fix is what runs) + python3
...
// sandbox-installed `openswarm` (the harness spec's default bin) — NOT the local-machine bin. Only
...
// A C toolchain baked FIRST: openswarm's local sibling dep `skill-tree` pulls `better-sqlite3`, a
...
const FIXIT_READY_CMD = "/opt/node/bin/openswarm --version && python3 -m pytest --version";
...
// Bake ONE generic template: build toolchain + node + the packed local openswarm, then pytest
...
if (process.env.RUN_FIXIT) {
  ...
  console.error(
    `[fixit] dry: set RUN_FIXIT=1 to run.\n` +
      ...
      `  model    : ${FIXIT_MODEL}\n` +
      `  agent bin: ${E2B ? "openswarm (sandbox-installed)" : FIXIT_HARNESS_BIN}\n` +
      `  mode     : ${FAKE ? "FAKE (no-LLM mock)" : `REAL (needs ${E2B ? "E2B_API_KEY + " : "openswarm on PATH + "}${creds})`}\n` +
