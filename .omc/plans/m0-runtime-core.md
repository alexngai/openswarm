...
Build the runtime foundation for a single atomic swarm-coder agent backed by `@anthropic-ai/claude-agent-sdk`. At exit, `swarm-coder prompt "..."` completes a real tool-using conversation end-to-end in both API-key and Claude-subscription paths.
...
- `swarm-coder login` subcommand — document-only; user runs `claude auth login` or `claude setup-token` themselves
...
5. **Anthropic TOS:** _"Anthropic does not allow third-party developers to offer claude.ai login or rate limits for their products, including agents built on the Claude Agent SDK."_ → swarm-coder owns **zero auth code**. Users run `claude auth login` themselves; we inherit credentials.
...
1. `ANTHROPIC_API_KEY=sk-ant-... npx swarm-coder prompt "say hi"` returns a text response and prints it to the TTY; exits 0.
2. `ANTHROPIC_API_KEY=sk-ant-... npx swarm-coder prompt "read the file package.json and count its keys"` invokes `read_file` through our dispatcher (observed in JSONL when `--headless`) and returns the model's answer.
3. `npx swarm-coder --permission-mode read-only prompt "write a test.txt file"` → permission denial surfaced as a tool_result error; model adapts or gives up; exit 0. No file created.
4. `npx swarm-coder --headless prompt "say hi"` emits a parseable JSONL stream to stdout; every line is a valid JSON object matching `NormalizedEvent` | `LaneEvent`; stream terminates with a `message_stop` event.
5. `npx swarm-coder --resume latest prompt "and one more thing"` resumes the most recent session in the current worktree and continues the conversation.
6. `npx swarm-coder doctor` on a clean install with only `ANTHROPIC_API_KEY` set prints 4 checks (auth / config / install / workspace), all PASS, exits 0. With no auth configured, auth check fails with actionable message pointing at `claude auth login` or `ANTHROPIC_API_KEY`.
7. `npx swarm-coder doctor --output-format json` emits valid JSON with `{checks: [{name, status, message}, ...], overall: "pass"|"fail"}`.
8. `npx swarm-coder init` in an empty dir creates `.swarm-coder/`, adds entries to `.gitignore`, and creates a stack-detected `CLAUDE.md` if one doesn't exist; idempotent.
...
4.1. `src/engine/claude-agent-sdk.ts` — `ClaudeAgentSdkEngine` implements `AgentEngine`.
  ...
  - `run(config)`:
    1. Build an in-process MCP server via `createSdkMcpServer({ name: "swarm-coder", tools: <our tools as SDK tool() defs> })`. Each tool's handler delegates to `config.executeTool` (set below), passing through the SDK's MCP Zod-validated input.
...
7.4. `src/cli/init.ts` — scaffold:
  - `.swarm-coder/` directory
  - `.gitignore` entries for `.swarm-coder/` and `node_modules/`
...
| Anthropic TOS enforcement changes | Low | High | swarm-coder owns no auth code; README explicitly says "run `claude auth login` yourself." Revisit if TOS changes. |
...
- Phase 7: `npx swarm-coder doctor` runs, prints 4 checks. `npx swarm-coder init` in `/tmp/test` scaffolds correctly
- Phase 8: `npx swarm-coder --headless prompt "hi" | head -1` is valid JSON
